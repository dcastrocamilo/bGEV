...
rmse.bgev30 = rmse.bgev50 = rmse.bgev100 = rmse.gev30 = rmse.gev50 = rmse.gev100 = rep(NA, nrow(scenarios))
for(i in 1:nrow(scenarios)){
  nn  = scenarios$n[i]
  rl = evd::qfrechet(1/rp, mu, nn^(1/alpha)*sigma, alpha) # rls at the limit
  
  rmse.bgev30[i]  = round(sqrt(mean((rl.bgev30[,i] - rl[1])^2)),2)
  rmse.bgev50[i]  = round(sqrt(mean((rl.bgev50[,i] - rl[2])^2)),2)
  rmse.bgev100[i] = round(sqrt(mean((rl.bgev100[,i] - rl[3])^2)),2)
  
  rmse.gev30[i]   = round(sqrt(mean((rl.gev30[,i] - rl[1])^2)),2)
  rmse.gev50[i]   = round(sqrt(mean((rl.gev50[,i] - rl[2])^2)),2)
  rmse.gev100[i]  = round(sqrt(mean((rl.gev100[,i] - rl[3])^2)),2)
  
}
cbind(scenarios, rmse.bgev30)
